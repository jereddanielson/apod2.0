...
class APP extends React.Component {
  ...
  state = { date: {}, data: {}, showHiRes: false, initialImageLoaded: false };
      ...
      >
        ...
        {(() => {
          if (this.state.showHiRes) {
            return (
              <HiRes
                url={this.state.data.hdurl}
                toggleHiRes={this.toggleHiRes}
              />
            );
          } else {
            return (
        ...
        {Moment(this.state.date)
          .add(1, "days")
          .isSameOrAfter(this.latestPossibleDate) ? undefined : this.state
            .initialImageLoaded ? (
          <ArrowNav direction="next" setNewDate={this.loadNextEntry} />
        ) : undefined}
        {this.state.initialImageLoaded ? (
          <ArrowNav direction="prev" setNewDate={this.loadPrevEntry} />
        ) : undefined}
        <Info />
      </div>
    );
  ...
  loadInitialImage = () => {
    ...
    if (
      dateToLoad.isValid() &&
      dateToLoad.isSameOrBefore(this.latestPossibleDate) &&
      dateToLoad.isSameOrAfter(this.earliestPossibleDate)
    ) {
  ...
  loadEntry = (_date) => {
    var dateToLoad = _date.isSameOrBefore(this.earliestPossibleDate)
      ? this.earliestPossibleDate.clone()
      : _date;
    dateToLoad = dateToLoad.isSameOrAfter(this.latestPossibleDate)
      ? this.latestPossibleDate.clone()
      : dateToLoad;
